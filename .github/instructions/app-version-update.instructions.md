---
description: "Use when updating app versions, bumping image tags, or modifying config.json/docker-compose.json for any app in the apps/ directory."
---
...
2. Update the image tag in `docker-compose.json` to match
3. Increment `tipi_version` by 1 in `config.json`
4. Update `updated_at` to current timestamp in milliseconds (`Date.now()`)

**The `version` in config.json and the image tag in docker-compose.json MUST be character-for-character identical.**
